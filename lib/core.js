...
let pool;
// Strip `sslmode` from the connection string so pg-connection-string doesn't emit the
// "SSL modes … treated as aliases for verify-full" deprecation warning (pg v9 change).
// We set SSL explicitly via the ssl option below, so behaviour is unchanged.
...
export function db() {
  if (!pool) pool = new Pool({ connectionString: pgConnString(), ssl: { rejectUnauthorized: false }, max: 3 });
  return pool;
}
...
export async function ensureSchema(d) {
  ...
  await d.query(`CREATE TABLE IF NOT EXISTS stores (
    ...
    first_party_domain TEXT,                   -- optional per-brand first-party pixel host (e.g. track.brand.com)
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now()
  )`);
  await d.query(`ALTER TABLE stores ADD COLUMN IF NOT EXISTS first_party_domain TEXT`).catch(() => {});
  await d.query(`ALTER TABLE stores ADD COLUMN IF NOT EXISTS meta_push_paused BOOLEAN DEFAULT false`).catch(() => {});
  ...
  await d.query(`CREATE TABLE IF NOT EXISTS system_health (
    k TEXT PRIMARY KEY,
  ...
  const { rows } = await d.query(
    `SELECT * FROM stores
       WHERE lower(shop_domain) = $1
          OR (coalesce(storefront,'') <> '' AND lower(storefront) = $1)
          OR (coalesce(storefront,'') <> '' AND $1 LIKE '%.' || lower(storefront))
          OR (coalesce(storefront,'') <> '' AND lower(storefront) LIKE '%.' || $1)
       ORDER BY (lower(shop_domain) = $1) DESC,
                (coalesce(storefront,'') <> '' AND lower(storefront) = $1) DESC
       LIMIT 1`, [dom]);
  return rows[0] || null;
}
